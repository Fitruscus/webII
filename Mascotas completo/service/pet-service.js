...
const buscarPets = (campo, valor) => {
    ...
    if (campo === 'id_dueno') {
        return fetch(`${API_PETS_URL}?select=*,clientes(nombre)&clientes.nombre=ilike.%25${encodeURIComponent(valor)}%25`, {
            headers: HEADERS
        })
        .then(res => res.json())
        .then(data => data.map(pet => ({
    ...
    return fetch(`${API_PETS_URL}?${campo}=ilike.%25${encodeURIComponent(valor)}%25&select=*,clientes(nombre)`, {
        headers: HEADERS
    })
    .then(res => res.json())
    .then(data => data.map(pet => ({
